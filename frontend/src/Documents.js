  ...
  const handleFiles = async (files) => {
    ...
    try {
      for (const file of fileArray) {
        ...
        // Validate file type
        const allowedTypes = ['.pdf', '.txt', '.csv', '.docx', '.doc', '.xlsx', '.xls', '.md'];
        const fileExt = '.' + file.name.split('.').pop().toLowerCase();
        if (!allowedTypes.includes(fileExt)) {
          setError(`${file.name} - File type not supported. Allowed: ${allowedTypes.join(', ')}`);
          continue;
        }
      ...
      <div className="documents-container">
        ...
        <div style={{ 
          ...
          <div>
            ...
            Document upload and management is restricted to admin users. You can view available documents and chat about them.
          </div>
        </div>
        ...
        {documents.length > 0 ? (
                ...
                <div key={doc.id} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '14px', color: '#6b7280', marginBottom: '8px' }}>
                      Size: {formatFileSize(doc.file_size)} • Type: {doc.file_type}
                    ...
                    <div style={{ fontSize: '12px', color: '#9ca3af', marginTop: '8px' }}>
                      Available since: {new Date(doc.created_at).toLocaleString()}
                    </div>
                  </div>
                  ...
                  <div style={{
                    ...
                    {doc.processed ? '✅ Available for Chat' : '⏳ Processing...'}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ 
            ...
            No documents available yet. Contact your admin to upload documents for the knowledge base.
          </div>
        )}
        ...
        <div style={{
          ...
          💡 <strong>Tip:</strong> You can ask questions about these documents in the Chat section. The AI will search through the uploaded content to provide relevant answers.
        </div>
      </div>
    );
  }
      ...
      >
        ...
        <div>
          ...
          <h3 style={{ margin: '0 0 10px 0', color: '#374151' }}>
            {dragActive ? 'Drop files here!' : isAdmin ? 'Upload Documents' : 'Admin Access Required'}
          </h3>
          <p style={{ margin: '0 0 15px 0', color: '#6b7280' }}>
            {isAdmin ? 'Drag and drop files here, or click to browse' : 'Only admin users can upload documents'}
          </p>
          {isAdmin && (
          ...
          <p style={{ margin: '15px 0 0 0', fontSize: '14px', color: '#9ca3af' }}>
